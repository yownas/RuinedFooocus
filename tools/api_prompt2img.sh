...
EVENT_ID=$(curl -sX POST ${BASEURL}/call/prompt2img -s -H "Content-Type: application/json" -d "$DATA" | awk -F'"' '{print $4}')

sleep 1 # Give RF some time to get things started

# Get the last "data" and base64 decode it.
curl -sN ${BASEURL}/call/prompt2img/$EVENT_ID | grep '^data:' | tail -1 | sed 's/^data: \["//;s/"\]$//' | base64 -d
