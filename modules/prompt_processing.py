        ...
        for i in range(0, steps):
            try:
                ...
                while "[" in prompt_per_step[i]:
                    ...
                    switchpattern = r"\[(.*?)\]"
                    allswitchpatterns = re.findall(switchpattern, startoflastpatterncomplete)
                    ...
                    for switchpattern in allswitchpatterns:
                        ...
                        for match in matches:
                            ...
                            if (intnumber != 0 and intnumber > i) or (intnumber == 0 and int(steps*decnumber) > i):
                            ...
                            if "|" in replacement or "~" in replacement or "^" in replacement or "?" in replacement or "/" in replacement or "\\" in replacement:
                                replacement = "[" +  replacement + "]"
                            prompt_to_append = prompt_per_step[i].replace(exact_match, replacement, 1)
                        ...
                        if matchfound == False:
                            for match in matches:
                                ...
                                if ":" in match.group(1):
                                    ...
                                    startdecnumber = float(number[1]) if len(parts) > 1 else 0
                                ...
                                if (intnumber != 0 and intnumber > i) or (intnumber == 0 and int(steps*decnumber) > i):
                                    if (startintnumber != 0 and startintnumber <= i) or (startdecnumber != 0 and int(steps*startdecnumber) <= i) or (startintnumber == 0 and startdecnumber == 0):
                        ...
                        if "~" in switchpattern:
                            ...
                            for match in matches:
                                ...
                                factor = max(round(steps/10),2) # minimum of 2, else just use |
                        ...
                        if "?" in switchpattern:
                            ...
                            for match in matches:
                                ...
                                for options, exact_match in zip(options_list, exact_matches):
                                    replacement = options[random.randint(0,len(options)-1)] # take a random value
                        ...
                        if "|" not in switchpattern and "~" not in switchpattern and "^" not in switchpattern and "?" not in switchpattern and "/" not in switchpattern and "\\" not in switchpattern and ":" not in switchpattern:
                            replacement = switchpattern
                            replacement = replacement.replace("[", "")
                            replacement = replacement.replace("]", "")

                            prompt_to_append = prompt_per_step[i]
                            prompt_to_append = prompt_to_append.replace(switchpattern, replacement, 1)
                            prompt_per_step[i] = prompt_to_append
